...
function performMovement() {
  ...
  try {
    if (bot.gameMode === 3) {
      // Spectator mode - simple flying
      spectatorMovement();
    } else {
      // Survival/Creative mode - simple walking
      survivalMovement();
    }
    movementCount++;
  } catch (err) {
...
function startBot() {
  ...
  bot.once('spawn', () => {
    ...
    // Setup pathfinder for survival mode
    if (bot.gameMode !== 3) {
  ...
  bot.on('game', () => {
    ...
    if (bot.gameMode === 3) {
      ...
      sendDiscordEmbed('Mode Change', `${botOptions.username} entered game mode ${bot.gameMode}.`, INFO_EMBED_COLOR);
...
app.get('/api/status', (req, res) => {
  try {
    ...
    const botStatus = {
      ...
      onlinePlayersCount,
      playerDetails,
      gameMode: bot?.gameMode === 3 ? "Spectator" : (bot?.gameMode !== undefined ? `Mode ${bot.gameMode}` : "Unknown"),
