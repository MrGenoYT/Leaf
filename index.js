...
function antiAFKAction() {
 if (!bot || !bot.entity) return;
 if (bot.gameMode === 3) return; // Skip anti-AFK in spectator mode

 const currentPosition = bot.entity.position;
 
 // Check Y-axis limits and use pathfinding only for non-spectator modes
...
// Disable block breaking and placing for spectator mode
function disableBlockInteractions() {
 if (!bot) return;
 
 // Override dig/break block methods to prevent them in spectator mode
 const originalDig = bot.dig;
 bot.dig = function(block, forceLook, digFace) {
   if (bot.gameMode === 3) {
     console.log('Block breaking disabled in spectator mode');
     return Promise.resolve();
   }
   return originalDig.call(this, block, forceLook, digFace);
 };
 
 const originalActivateBlock = bot.activateBlock;
 bot.activateBlock = function(block, direction, cursorPos) {
   if (bot.gameMode === 3) {
     console.log('Block interaction disabled in spectator mode');
     return Promise.resolve();
   }
   return originalActivateBlock.call(this, block, direction, cursorPos);
 };
 
 const originalPlaceBlock = bot.placeBlock;
 bot.placeBlock = function(referenceBlock, faceVector, cb) {
   if (bot.gameMode === 3) {
     console.log('Block placing disabled in spectator mode');
     if (cb) cb(new Error('Block placing disabled in spectator mode'));
     return;
   }
   return originalPlaceBlock.call(this, referenceBlock, faceVector, cb);
 };
}
...
function startBot() {
 ...
 // Disable block interactions immediately after bot creation
 ...
 bot.once('spawn', () => {
   ...
   // Configure pathfinder to not break blocks
   if (bot.pathfinder) {
     const movements = new Movements(bot, bot.mcData);
     movements.canDig = false; // Disable digging
     movements.canPlaceOn = false; // Disable placing blocks
     movements.blocksCantBreak.add(bot.mcData.blocksByName.bedrock.id);
     // Add all blocks to the "can't break" list to be extra safe
     Object.values(bot.mcData.blocksByName).forEach(block => {
 ...
 bot.on('game', () => {
   if (bot.gameMode === 3) {
       ...
       if (wasNotSpectator) {
           ...
           // Stop any pathfinding when entering spectator mode
           if (bot.pathfinder) {
             bot.pathfinder.stop();
           }
       ...
       // Re-enable pathfinding for non-spectator modes
       if (bot.pathfinder) {
         const movements = new Movements(bot, bot.mcData);
         movements.canDig = true; // Re-enable digging for survival/creative modes
         movements.canPlaceOn = true; // Re-enable placing blocks
         bot.pathfinder.setMovements(movements);
       }
