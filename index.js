...
function startSpectatorMovement() {
  ...
  clearInterval(moveInterval);
  moveInterval = setInterval(moveRandomly, 1000);
  moveRandomly();
}
...
function disableAllBlockInteractions() {
  if (!bot) return;
  
  const noOpHandler = () => {
    console.log('Block interaction disabled');
    return Promise.resolve();
  };

  const noOpCallbackHandler = (callback) => {
    console.log('Block interaction disabled');
    if (callback) callback(new Error('Block interaction disabled'));
    return;
  };

  bot.dig = noOpHandler;
  bot.activateBlock = noOpHandler;
  bot.placeBlock = noOpCallbackHandler;
  bot.activateItem = noOpHandler;
  bot.deactivateItem = noOpHandler;
  bot.useOn = noOpHandler;
  bot.attack = noOpHandler;
  bot.swingArm = noOpHandler;
  ...
  if (bot._client) {
    ...
    bot._client.write = function(name, params) {
      ...
      if (blockedPackets.includes(name)) {
        console.log(`Blocked packet: ${name}`);
        return;
      }
