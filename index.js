...
// Spectator mode movement: random flying in XZ plane, Y altitude remains the same.
function spectatorMovement() {
  if (!bot || !bot.entity) return; // Ensure bot and its entity are available
  const currentPos = bot.entity.position;
  
  // Calculate new X and Z coordinates for movement
  const x = currentPos.x + (Math.random() * 10 - 5); // Move +/- 5 blocks in X
  const z = currentPos.z + (Math.random() * 10 - 5); // Move +/- 5 blocks in Z
  const currentY = currentPos.y; // Maintain the current Y altitude

  // Update bot's position directly for spectator mode
  bot.entity.position.set(x, currentY, z);

  console.log(`Spectator fly to: ${Math.floor(x)}, ${Math.floor(currentY)}, ${Math.floor(z)}`);
}
...
bot.on('playerJoined', (player) => {
    if (player.username !== botOptions.username) {
      ...
      sendChatEmbed('Player Joined', `**${player.username}** joined the 
game.`, SUCCESS_EMBED_COLOR, [
